...
public class Bunkers : MonoBehaviour
{
    ...
    //pri resetu se nastavi puvodni bunkder
    ...
    public bool CheckCollision(BoxCollider2D other, Vector3 hitPoint)
    {
        ...
        return Splat(hitPoint) || Splat(hitPoint + (Vector3.down * offset.y)) || Splat(hitPoint + (Vector3.up * offset.y)) || Splat(hitPoint + (Vector3.left * offset.x)) || Splat(hitPoint + (Vector3.right * offset.x));
    }
